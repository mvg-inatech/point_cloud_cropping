...
from dataset.utils import offset2bincount, dict_rename_for_pointcept
...
class Block(PointModule):
    def __init__(
        ...
        if layer_scale is not None:
            self.ls1 = PointSequential(LayerScale(channels, init_values=layer_scale))
    ...
    def forward(self, point: Point):
        ...
        attn = self.attn(point)
        if hasattr(self, "ls1"):
            attn = self.ls1(attn)
        point = self.drop_path(attn)
        point.feat = shortcut + point.feat
...
class PointTransformerV3(PointModule):
    def __init__(
        ...
        shuffle_orders = pt_config.shuffle_orders
        enable_rpe = pt_config.enable_rpe
        enable_flash = pt_config.enable_flash
        upcast_attention = pt_config.upcast_attention
        upcast_softmax = pt_config.upcast_softmax
